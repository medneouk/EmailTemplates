...
# ADDED BY SIMON JACKSON
## Addressing the line-wrapping issue with Landlines and Mobiles;
# following advice here: https://github.com/Set-OutlookSignatures/Set-OutlookSignatures/tree/main?tab=readme-ov-file#1718-how-to-avoid-blank-lines-when-replacement-variables-return-an-empty-string
$ReplaceHash['$CurrentUserTelephone-prefix-noempty$'] = $(
    if (-not $ReplaceHash['$CurrentUserTelephone$']) { '' }
    else {
        $(
            if ($UseHtmTemplates) { '<br>' }
            else { "`r" } #carrage return is larger than a new line
        )
    }
)
$ReplaceHash['$CurrentUserMobile-prefix-noempty$'] = $(
    if (-not $ReplaceHash['$CurrentUserMobile$']) { '' }
    else {
        $(
            if ($UseHtmTemplates) { '<br>' }
            else { "`n" } #new line touches the line above
        )
    }
)